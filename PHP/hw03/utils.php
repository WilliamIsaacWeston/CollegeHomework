...
function highlight_file_between($filename, $starttime, $endtime) {
  if (strtotime($starttime) <= time() && strtotime($endtime) >= time()) { 
    ...
    echo "    <p>\n      Source files are only viewable between $starttime and $endtime.\n    </p>\n";
  }
}
...
function getInput($name){
  $formInput = htmlspecialchars(trim( $_POST[$name] ));
 ...
 * @return the input from a user form,
 */
function getArrayInput($name){
  $arrToString = arrayToString($_POST[$name]);
